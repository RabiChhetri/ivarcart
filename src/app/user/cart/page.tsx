...
import {increaseQuantity,decreaseQuantity,removeFromCart,calculateTotals  } from "@/redux/cartSlice";
    ...
    <div className="w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative">
      ...
      {cartData.length == 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-5">
            <AnimatePresence>
              {cartData.map((item, index) => (
                ...
                >
                    <div className="relative w-28 h-28 sm:w-24 sm:h-24 md:w-28 md:h-28 flex-shrink-0 rounded-xal overflow-hidden bg-gray-50">
                ...
                <div className="mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left">
                  ...
                  <p className="text-green-700 font-bold mt-1 text-sm sm:text-base">रु {Number(item.price)*item.quantity}</p>
                </div>

                <div className="flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full">
                  <button className="bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border-gray-200" onClick={() => dispatch(decreaseQuantity(item._id))}><Minus size={14} className="text-green-700"/></button>
                  <span className="font-semibold text-gray-800 w-6 text-center">{item.quantity}</span>
                  <button className="bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border-gray-200"  onClick={() => dispatch(increaseQuantity(item._id))} ><Plus size={14} className="text-green-700"/></button>
                </div>
          ...
          transition={{duration:0.3}}
          className="bg-white rounded-2xl shadow-xl p-6 h-fit sticky top-24 border-gray-100 flex flex-col"
          >
