...
function Navbar({ user }: { user: IUser }) {
  ...
  const sideBar = menuOpen
    ? createPortal(
      ...
      bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,225,100,0.3)] flex flex-col p-6 text-white"
          >
            <div className="flex justify-between items-center mb-2">
              ...
              <button
                className="text-white/80 hover:text-red-400 text-2x font-bold transition"
    ...
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50">
      ...
      <div className="flex items-center gap-3 md:gap-6 relative">
        {user.role == "user" && (
          <>
            ...
            >
              ...
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 flex justify-center items-center rounded-full font-semibold shadow">
                {cartData.length}
              </span>
            </Link>
          </>
        ...
        <div className="relative" ref={profileDropDown}>
          {/* Avatar button */}
          <div
            className="relative w-11 h-11 rounded-full overflow-hidden bg-white shadow-md hover:scale-105 transition-transform cursor-pointer"
          ...
          <AnimatePresence>
            {open && (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-3 py-2 border-b border-gray-100">
                  <div className="relative w-10 h-10 rounded-full overflow-hidden bg-green-100">
                    {user.image ? (
                      ...
                      <User className="w-5 h-5 text-green-700 m-auto" />
                    )}
                  </div>
                  <div>
                ...
                {/* Logout */}
                <button
                  className="flex items-center gap-2 w-full text-left px-3 py-3 hover:bg-green-50 rounded-lg text-gray-700 font-medium"
                  onClick={() => {
                    setOpen(false);
