import { ArrowLeft, Eye, EyeIcon, EyeOff, Key, Leaf, Loader2, LogIn, Mail, User } from 'lucide-react'
import React, { useState } from 'react'
import {isForcedMotionValue, motion} from 'motion/react'
...
const RegisterForm = ({previousStep}:propType) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative'>
      ...
      <p className='text-gray-600 mb-8 flex items-center'>Join Ivarcart today <Leaf className='w-5 h-5 text-green-600'/> </p>
...
<div className='w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-50 py-3 rounded-xl text-gray-700 font-medium transition-all duration-200' onClick={() => signIn("google",{callbackUrl:'/'})}>
  ...
  Continue with Google
      ...
      <p className='cursor-pointer text-gray-600 mt-6 text-sm flex items-center gap-1' onClick={()=>{
        router.push("/login")
